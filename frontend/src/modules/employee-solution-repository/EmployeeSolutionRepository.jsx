...
const EmployeeSolutionRepository = () => {
  ...
  const fetchSolutions = useCallback(async () => {
    try {
      setLoading(true);
      console.log("[DEBUG] fetchSolutions - Calling API: /solutions/my-repository");
      const res = await axiosInstance.get("/solutions/my-repository");
  ...
  const statistics = useMemo(() => {
    ...
    const approved = solutions.filter((s) => s.reviewStatus === "APPROVED").length;
    const rework = solutions.filter((s) => s.reviewStatus === "REWORK").length;
    const final = solutions.filter((s) => s.solutionType === "FINAL").length;
    const paraphrase = solutions.filter((s) => s.solutionType === "PARAPHRASE").length;
  ...
  const filteredSolutions = useMemo(() => {
    const filtered = solutions.filter((solution) => {
      ...
      return matchesSearch && matchesType && matchesStatus && matchesSubmittedDate && matchesReviewedDate;
    });
    ...
    console.log("[DEBUG] filteredSolutions - Filters:", { searchText, solutionTypeFilter, reviewStatusFilter, submittedDateFilter, reviewedDateFilter });

    return filtered;
  }, [solutions, searchText, solutionTypeFilter, reviewStatusFilter, submittedDateFilter, reviewedDateFilter]);
  ...
  return (
    <EmployeeLayout>
      <PageHeader
        ...
        subtitle="View all your reviewed solutions - Approved and Rework."
      />
                ...
                {filteredSolutions.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={10} align="center">
                      <Typography py={5} color="text.secondary">
                        No reviewed solutions available.
                      </Typography>
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredSolutions.map((solution) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Chip label={solution.reviewStatus} color={getStatusColor(solution.reviewStatus)} size="small" />
                      </TableCell>
          ...
          {selectedSolution && (
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6}>
                ...
                <Chip label={selectedSolution.reviewStatus} color={getStatusColor(selectedSolution.reviewStatus)} size="small" />
              </Grid>
              <Grid item xs={12} md={6}>
