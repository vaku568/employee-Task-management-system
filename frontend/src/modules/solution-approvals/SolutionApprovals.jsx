...
const SolutionApprovals = () => {
  ...
  const fetchSolutions = useCallback(async () => {
    try {
      ...
      // Filter only approved solutions
      const approvedSolutions = res.data.filter((sol) => sol.reviewStatus === "APPROVED");
  ...
  useEffect(() => {
    ...
    if (socket) {
      const handleSolutionSubmitted = (data) => {
        console.log("[SolutionApprovals] Solution submitted:", data);
        fetchSolutions();
      };

      const handleSolutionApproved = (data) => {
        console.log("[SolutionApprovals] Solution approved:", data);
        fetchSolutions();
      };

      const handleSolutionRework = (data) => {
        console.log("[SolutionApprovals] Solution rework:", data);
        fetchSolutions();
      };
  ...
  const statistics = useMemo(() => {
    ...
    const todayApprovals = solutions.filter((sol) => isToday(sol.reviewedAt || sol.submittedAt)).length;
    const weekApprovals = solutions.filter((sol) => isThisWeek(sol.reviewedAt || sol.submittedAt)).length;
    const uniqueEmployees = new Set(solutions.map((sol) => sol.employeeId?._id).filter(Boolean)).size;
  ...
  const filteredSolutions = useMemo(() => {
    return solutions.filter((solution) => {
      ...
      return matchesSearch && matchesTeam && matchesType && matchesEmployee && matchesSubmittedDate && matchesReviewedDate;
    });
  }, [solutions, searchText, teamFilter, solutionTypeFilter, employeeFilter, submittedDateFilter, reviewedDateFilter]);
  ...
  return (
    <TeamLeadLayout pageTitle="Solution Repository">
      <PageHeader
        ...
        subtitle="View and manage all approved solutions from employees."
      />
      ...
      >
        {[
          ...
          { title: "Today's Approvals", value: statistics.todayApprovals, color: "#16A34A" },
          { title: "This Week's Approvals", value: statistics.weekApprovals, color: "#F59E0B" },
          { title: "Employees Contributed", value: statistics.uniqueEmployees, color: "#7C3AED" },
        ].map((item) => (
          <Box
            key={item.title}
            sx={{
          ...
          {selectedSolution && (
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6}>
                ...
                <Chip label={selectedSolution.reviewStatus} color="success" size="small" />
              </Grid>
              <Grid item xs={12} md={6}>
...
export default SolutionApprovals;